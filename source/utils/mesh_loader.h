...
obj_load_mesh(const std::string &path, u64 props)
{
  ...
  if (props & mesh_loader_properties_normals)
	...
	{
		auto pre = i*data_offset + color_pre_offset;
		buffer_ptr[pre + 0] = attrib.texcoords[2 * i + 0];
		buffer_ptr[pre + 1] = attrib.texcoords[2 * i + 1];
	}
	...
	return (renderer::mesh_t){"", vbo, ibo, shapes[0].mesh.indices.size()};
}
